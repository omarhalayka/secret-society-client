...
// نستخدم HTML Audio مش Phaser Sound عشان يضل شغال بين الـ scenes
...
class AudioManager {
    ...
    private constructor() {
        ...
        this.audio.volume = 0.28;   // هادئ - أقل من النص بثلاث درجات
    ...
    // ─── شغّل الموسيقى (لازم يكون بعد user interaction) ───
    play() {
        if (!this.isMuted) {
            this.audio.play().catch(() => {
                // بعض المتصفحات تمنع التشغيل التلقائي
                // بنتشتغل أول ما المستخدم يتفاعل
            });
        }
    }

    // ─── أنشئ زر الـ mute الثابت ───
    createMuteButton() {
